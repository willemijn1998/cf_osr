...
def ocr_test(args, lvae, train_loader, test_loader_seen, test_loader_unseen, writer):
    ...
    # SAVE HYPERPARAMETERS AND PERFORMANCE HERE (incl precision recall for different settings)
    hparam_list = ["run_idx", "lamda", "threshold", "encode_z", "mmd_loss", "s_jitter", 
    "p_grayscale", "contrastive_loss", "supcon_loss", "gamma", "eta", "theta", "con_temperature"]
    
    config = vars(args)
    hparam_dict = {i: config[i] for i in hparam_list}
    metric_dict = {'Precision': perf_test[0], 'Recall': perf_test[1], "F1-score": perf_test[2]}
    run_name = "runs/%s"%(args.exp_name)
    ...
    if not os.path.exists(save_path_father):
        ...
        with open(save_path_father, "w") as f:
            ...
            print('written f-score at {} with {} '.format(save_path_father, performance_string))
    else:
        performance_string = str(args.exp_name)+ ', threshold '+ str(args.threshold)+': ' + str(perf_test[2])
